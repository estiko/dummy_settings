...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("xoplax_lock")) {
            resId = R.string.lockscreen_lock_icon_xoplax;
    ...
    private void resizeXoplaXLock() {
        Bitmap xoplaxLock = BitmapFactory.decodeResource(getResources(), R.drawable.xoplax_lock);
        if (xoplaxLock != null) {
            String path = null;
            int px = requestImageSize();
            xoplaxLock = Bitmap.createScaledBitmap(xoplaxLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "xoplax_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                xoplaxLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
